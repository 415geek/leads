...
export async function POST(request: Request) {
  if (!isLeadEvidenceCrossValidateEnabled()) {
    return NextResponse.json(
      {
        error: '证据链交叉验证未启用',
        hint: '设置 ENABLE_LEAD_EVIDENCE_CROSS_VALIDATE=1，并在 Supabase 执行 lead_evidence 迁移后再试。',
      },
      { status: 503 },
    );
  }
  ...
  try {
    ...
    const result = await crossValidateLeadById(supabaseAdmin, leadId);
    ...
    console.error('[POST /api/leads/cross-validate]', err);
    return NextResponse.json({ error: '交叉验证失败，请稍后重试' }, { status: 500 });
  }
}
