...
- **dev-prompts 升级**（`docs/DEV_PROMPTS_ROADMAP.md`）：P1a `lead_evidence` 迁移；P2 `lib/enrichment/*`（skip-trace）；P3 `POST /api/leads/cross-validate` 默认关（`ENABLE_LEAD_EVIDENCE_CROSS_VALIDATE=1`）。**勿重建 `lead_contacts`**。Supabase 迁移需手动 SQL Editor。
